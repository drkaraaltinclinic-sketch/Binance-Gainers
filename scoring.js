...
//
// Tokenomics used to be a hard veto here, back when this app was built
// entirely around unlock-chasing (where tokenomics data was the whole
// point). Now that the focus is Binance Gainers — a momentum/funding/order
// book thesis — tokenomics is useful context (market cap for position
// sizing, VWAP for the squeeze score) but was never actually load-bearing
// for the entry decision itself. Missing it now degrades gracefully
// instead of blocking every ticker.
function gateConviction({ squeezeScore, perpListed, tokenomicsOk, tokenomicsReason }) {
  ...
  if (!tokenomicsOk) {
    notes.push(
      `Tokenomics data unavailable${tokenomicsReason ? ` (${tokenomicsReason})` : ""} — market cap tier defaults to most conservative, VWAP signal skipped`
    );
  }
  if (squeezeScore < 20)
    vetoes.push(`Squeeze-probability too low (score ${squeezeScore}) to justify a short bias today`);

  return { cleared: vetoes.length === 0, vetoes, notes };
}
