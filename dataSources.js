...
// Cache of each symbol's 00:00 UTC daily open — it doesn't change until
// the next UTC midnight, so after the first press each day, re-presses
// only need the fresh batch ticker (one call), not a kline per symbol.
let _dayOpenCache = new Map(); // symbol -> { utcDate, dayOpen }

function utcDateKey() {
  return new Date().toISOString().slice(0, 10);
}

async function getDayOpen(symbol) {
  const key = utcDateKey();
  const cached = _dayOpenCache.get(symbol);
  if (cached && cached.utcDate === key) return cached.dayOpen;
  const kr = await fetchWithTimeout(`${FAPI}/fapi/v1/klines?symbol=${symbol}&interval=1d&limit=1`);
  if (!kr.ok) throw new Error(`klines ${kr.status}`);
  const kl = await kr.json();
  const dayOpen = parseFloat(kl[0][1]);
  _dayOpenCache.set(symbol, { utcDate: key, dayOpen });
  return dayOpen;
}

// Ranks by change SINCE TODAY'S 00:00 UTC OPEN — which is what Binance's
// own Futures "Gainers" list uses, confirmed by side-by-side comparison:
// tokens that were flat overnight matched under both definitions, tokens
// that pumped before midnight UTC showed inflated rolling-24h figures, and
// tokens that pumped yesterday showed up as phantom "gainers" on rolling
// while being absent from Binance's list entirely. Rolling 24h is still
// returned as secondary info.
//
// Superset approach: rank the whole market by rolling 24h first (one
// batch call), take the top 60, fetch/cached daily opens for those, then
// re-rank by UTC-day change. A top UTC-day gainer will almost always sit
// in the rolling top 60 too; the rare exception (crashed yesterday, bounced
// today) is a recovery, not the parabolic-squeeze pattern this hunts.
async function getTopGainers(limit = 10) {
  try {
    ...
    const superset = all
      ...
      .filter((t) => !isNaN(t.priceChangePercent) && t.lastPrice > 0)
      .sort((a, b) => b.priceChangePercent - a.priceChangePercent)
      .slice(0, 60);

    await Promise.all(
      superset.map(async (g) => {
        try {
          const dayOpen = await getDayOpen(g.symbol);
          g.utcDayOpen = dayOpen;
          g.utcDayChangePercent = dayOpen > 0 ? ((g.lastPrice - dayOpen) / dayOpen) * 100 : null;
        } catch {
          g.utcDayOpen = null;
          g.utcDayChangePercent = null;
        }
      })
    );

    const gainers = superset
      .filter((g) => g.utcDayChangePercent != null)
      .sort((a, b) => b.utcDayChangePercent - a.utcDayChangePercent)
      .slice(0, limit);

    return { ok: true, gainers, rankedBy: "utc_day_change" };
  } catch (err) {
    return { ok: false, reason: err.message };
  }
}
...
// Historical settled funding rates (Binance settles every 8h) — needed to
// detect a TREND (was rising, now flattening/dropping), not just a single
// current reading. This is the actual "funding rate flip" signal: shorting
// while funding is still climbing means entering mid-squeeze; the safer
// window is after it peaks and turns.
async function getFundingRateHistory(symbol, limit = 3) {
  try {
    const res = await fetchWithTimeout(`${FAPI}/fapi/v1/fundingRate?symbol=${symbol}&limit=${limit}`);
    if (!res.ok) throw new Error(`fundingRate history ${res.status}`);
    const j = await res.json();
    const rates = j.map((r) => ({
      time: r.fundingTime,
...
// Historical open interest — same idea: is OI still building (squeeze
// still forming) or has it flattened/dropped (positions closing, possible
// exhaustion)?
async function getOpenInterestHistory(symbol, period = "1h", limit = 6) {
  try {
    const res = await fetchWithTimeout(
      `${FAPI}/futures/data/openInterestHist?symbol=${symbol}&period=${period}&limit=${limit}`
    );
    if (!res.ok) throw new Error(`openInterestHist ${res.status}`);
    const j = await res.json();
    const points = j.map((p) => ({
      time: p.timestamp,
      openInterest: parseFloat(p.sumOpenInterest),
    }));
